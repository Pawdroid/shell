...
checkCentosSELinux() {
    if [[ -f "/etc/selinux/config" ]] && ! grep -q "SELINUX=disabled" <"/etc/selinux/config"; then
        ...
        echoContent yellow "https://v2cross.com/archives/5007"
        exit 0
    fi
}
checkSystem() {
...
readInstallType() {
    ...
    # 1.检测安装目录
    if [[ -d "/etc/v2cross" ]]; then
        # 检测安装方式 v2ray-core
        #        if [[ -d "/etc/v2cross/v2ray" && -f "/etc/v2cross/v2ray/v2ray" && -f "/etc/v2cross/v2ray/v2ctl" ]]; then
        #            if [[ -d "/etc/v2cross/v2ray/conf" && -f "/etc/v2cross/v2ray/conf/02_VLESS_TCP_inbounds.json" ]]; then
        #                configPath=/etc/v2cross/v2ray/conf/
        #                if grep </etc/v2cross/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "tls"'; then
        #                    coreInstallType=2
        #                    ctlPath=/etc/v2cross/v2ray/v2ctl
        #                fi
        #            fi
        #        fi

        if [[ -d "/etc/v2cross/xray" && -f "/etc/v2cross/xray/xray" ]]; then
            # 这里检测xray-core
            if [[ -d "/etc/v2cross/xray/conf" ]] && [[ -f "/etc/v2cross/xray/conf/02_VLESS_TCP_inbounds.json" || -f "/etc/v2cross/xray/conf/02_trojan_TCP_inbounds.json" || -f "/etc/v2cross/xray/conf/07_VLESS_vision_reality_inbounds.json" ]]; then
                # xray-core
                configPath=/etc/v2cross/xray/conf/
                ctlPath=/etc/v2cross/xray/xray
                coreInstallType=1
        ...
        if [[ -d "/etc/v2cross/hysteria" && -f "/etc/v2cross/hysteria/hysteria" ]]; then
            if [[ -d "/etc/v2cross/hysteria/conf" ]] && [[ -f "/etc/v2cross/hysteria/conf/config.json" ]]; then
                hysteriaConfigPath=/etc/v2cross/hysteria/conf/
            fi
        fi

        if [[ -d "/etc/v2cross/tuic" && -f "/etc/v2cross/tuic/tuic" ]]; then
            if [[ -d "/etc/v2cross/tuic/conf" ]] && [[ -f "/etc/v2cross/tuic/conf/config.json" ]]; then
                tuicConfigPath=/etc/v2cross/tuic/conf/
            fi
        fi

        if [[ -d "/etc/v2cross/sing-box" && -f "/etc/v2cross/sing-box/sing-box" ]]; then
            if [[ -d "/etc/v2cross/sing-box/conf" ]] && [[ -f "/etc/v2cross/sing-box/conf/config/tuic.json" || -f "/etc/v2cross/sing-box/conf/config/hysteria2.json" ]]; then
                #                coreInstallType=3
                singBoxConfigPath=/etc/v2cross/sing-box/conf/
            fi
        fi
...
readInstallProtocolType() {
    ...
    if [[ -n "${singBoxConfigPath}" ]] && [[ -f "/etc/v2cross/sing-box/conf/config.json" ]] && grep -q 'hysteria2' </etc/v2cross/sing-box/conf/config.json; then
        currentInstallProtocolType=${currentInstallProtocolType}'6'
    fi
    if [[ -n "${singBoxConfigPath}" ]] && [[ -f "/etc/v2cross/sing-box/conf/config.json" ]] && grep -q 'tuic' </etc/v2cross/sing-box/conf/config.json; then
        ...
        if [[ -d '/www/server/panel/vhost/cert/' && -n $(find /www/server/panel/vhost/cert/*/fullchain.pem) ]]; then
            ...
            if [[ -n "${selectBTDomain}" ]]; then
                ...
                if [[ -z "${btDomain}" ]]; then
                    ...
                    domain=${btDomain}
                    if [[ ! -f "/etc/v2cross/tls/${btDomain}.crt" && ! -f "/etc/v2cross/tls/${btDomain}.key" ]]; then
                        ln -s "/www/server/panel/vhost/cert/${btDomain}/fullchain.pem" "/etc/v2cross/tls/${btDomain}.crt"
                        ln -s "/www/server/panel/vhost/cert/${btDomain}/privkey.pem" "/etc/v2cross/tls/${btDomain}.key"
                    fi
...
# 读取Tuic配置
readSingBoxConfig() {
    if [[ -n "${singBoxConfigPath}" ]] && [[ -f "/etc/v2cross/sing-box/conf/config.json" ]]; then
        if grep -q 'tuic' </etc/v2cross/sing-box/conf/config.json; then
        ...
        if grep -q 'hysteria2' </etc/v2cross/sing-box/conf/config.json; then
...
unInstallSingBox() {
    local type=$1
    if [[ -n "${singBoxConfigPath}" ]] && [[ -f "/etc/v2cross/sing-box/conf/config.json" ]]; then
        if grep -q 'tuic' </etc/v2cross/sing-box/conf/config.json && [[ "${type}" == "tuic" ]]; then
            rm "${singBoxConfigPath}config/tuic.json"
            echoContent green " ---> 删除sing-box tuic配置成功"
        fi

        if grep -q 'hysteria2' </etc/v2cross/sing-box/conf/config.json && [[ "${type}" == "hysteria2" ]]; then
...
cleanUp() {
    ...
    #        rm -rf "$(find /etc/v2cross/v2ray/* | grep -E '(config_full.json|conf)')"
    #        handleV2Ray stop >/dev/null
    #        rm -f /etc/systemd/system/v2ray.service
    #    el
    if [[ "$1" == "xrayClean" ]]; then
        rm -rf "$(find /etc/v2cross/xray/* | grep -E '(config_full.json|conf)')"
        handleXray stop >/dev/null
        rm -f /etc/systemd/system/xray.service

    elif [[ "$1" == "v2rayDel" ]]; then
        rm -rf /etc/v2cross/v2ray/*

    elif [[ "$1" == "xrayDel" ]]; then
        rm -rf /etc/v2cross/xray/*
    fi
}
initVar "$1"
checkSystem
...
mkdirTools() {
    ...
    mkdir -p /etc/v2cross/subscribe_local/default
    mkdir -p /etc/v2cross/subscribe_local/clashMeta

    mkdir -p /etc/v2cross/subscribe_remote/default
    mkdir -p /etc/v2cross/subscribe_remote/clashMeta

    mkdir -p /etc/v2cross/subscribe/default
    mkdir -p /etc/v2cross/subscribe/clashMetaProfiles
    mkdir -p /etc/v2cross/subscribe/clashMeta

    mkdir -p /etc/v2cross/v2ray/conf
    mkdir -p /etc/v2cross/v2ray/tmp
    mkdir -p /etc/v2cross/xray/conf
    mkdir -p /etc/v2cross/xray/tmp
    mkdir -p /etc/v2cross/hysteria/conf
    mkdir -p /etc/systemd/system/
    mkdir -p /tmp/v2cross-tls/

    mkdir -p /etc/v2cross/warp

    mkdir -p /etc/v2cross/tuic/conf

    mkdir -p /etc/v2cross/sing-box/conf/config
}
...
installTools() {
    ...
    ${upgrade} >/etc/v2cross/install.log 2>&1
    if grep <"/etc/v2cross/install.log" -q "changed"; then
    ...
    if [[ "${selectCustomInstallType}" == "7" ]]; then
        ...
        if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
            ...
            curl -s https://get.acme.sh | sh >/etc/v2cross/tls/acme.log 2>&1

            if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
                echoContent red "  acme安装失败--->"
                tail -n 100 /etc/v2cross/tls/acme.log
...
initTLSNginxConfig() {
    ...
    if [[ -n "${currentHost}" ]]; then
        ...
        if [[ "${historyDomainStatus}" == "y" ]]; then
            ...
            echo
            echoContent yellow "请输入要配置的域名 例: www.v2cross.com --->"
            read -r -p "域名:" domain
        fi
    else
        echo
...
server {
	...
	location ~ ^/s/(clashMeta|default|clashMetaProfiles)/(.*) {
        ...
        alias /etc/v2cross/subscribe/\$1/\$2;
    }
...
switchSSLType() {
    if [[ -z "${sslType}" ]]; then
        ...
        *)
        ...
        echo "${sslType}" >/etc/v2cross/tls/ssl_type
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2cross/tls/acme.log >/dev/null

        local txtValue=
        txtValue=$(tail -n 10 /etc/v2cross/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
        if [[ -n "${txtValue}" ]]; then
            echoContent green " ---> 请手动添加DNS TXT记录"
            echoContent yellow " ---> 添加方法请参考此教程，https://github.com/Pawdroid/shell/blob/master/documents/dns_txt.md"
            ...
            if [[ "${addDNSTXTRecordStatus}" == "y" ]]; then
                ...
                if echo "${txtAnswer}" | grep -q "^${txtValue}"; then
                    ...
                    if [[ -n "${installSSLIPv6}" ]]; then
                        sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2cross/tls/acme.log >/dev/null
                    else
                        sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" 2>&1 | tee -a /etc/v2cross/tls/acme.log >/dev/null
                    fi
                else
        ...
        echoContent green " ---> 生成证书中"
        sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2cross/tls/acme.log >/dev/null
    fi
}
# 自定义端口
customPortFunction() {
...
installTLS() {
    ...
    # 安装tls
    if [[ -f "/etc/v2cross/tls/${tlsDomain}.crt" && -f "/etc/v2cross/tls/${tlsDomain}.key" && -n $(cat "/etc/v2cross/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
        echoContent green " ---> 检测到证书"
        # checkTLStatus
        renewalTLS

        if [[ -z $(find /etc/v2cross/tls/ -name "${tlsDomain}.crt") ]] || [[ -z $(find /etc/v2cross/tls/ -name "${tlsDomain}.key") ]] || [[ -z $(cat "/etc/v2cross/tls/${tlsDomain}.crt") ]]; then
            sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${tlsDomain}" --fullchainpath "/etc/v2cross/tls/${tlsDomain}.crt" --keypath "/etc/v2cross/tls/${tlsDomain}.key" --ecc >/dev/null
        else
            echoContent yellow " ---> 如未过期或者自定义证书请选择[n]\n"
            read -r -p "是否重新安装？[y/n]:" reInstallStatus
            if [[ "${reInstallStatus}" == "y" ]]; then
                rm -rf /etc/v2cross/tls/*
                installTLS "$1"
            fi
        fi
        ...
        #            echo
        #            if [[ -d "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.key" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.cer" ]]; then
        #                sudo "$HOME/.acme.sh/acme.sh" --installcert -d "*.${dnsTLSDomain}" --fullchainpath "/etc/v2cross/tls/${tlsDomain}.crt" --keypath "/etc/v2cross/tls/${tlsDomain}.key" --ecc >/dev/null
        #            fi
        #
        #        el
        ...
        if [[ ! -f "/etc/v2cross/tls/${tlsDomain}.crt" || ! -f "/etc/v2cross/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/v2cross/tls/${tlsDomain}.key") || -z $(cat "/etc/v2cross/tls/${tlsDomain}.crt") ]]; then
            tail -n 10 /etc/v2cross/tls/acme.log
            ...
            if tail -n 10 /etc/v2cross/tls/acme.log | grep -q "Could not validate email address as valid"; then
...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            wget -q -P "${nginxStaticPath}" https://raw.githubusercontent.com/Pawdroid/shell/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
updateSELinuxHTTPPortT() {
    ...
    $(find /usr/bin /usr/sbin | grep -w journalctl) -xe >/etc/v2cross/nginx_error.log 2>&1

    if find /usr/bin /usr/sbin | grep -q -w semanage && find /usr/bin /usr/sbin | grep -q -w getenforce && grep -E "31300|31302" </etc/v2cross/nginx_error.log | grep -q "Permission denied"; then
...
handleNginx() {
    ...
    if [[ -z $(pgrep -f "nginx") ]] && [[ "$1" == "start" ]]; then
        systemctl start nginx 2>/etc/v2cross/nginx_error.log
        ...
        if [[ -z $(pgrep -f "nginx") ]]; then
            ...
            if grep -q "journalctl -xe" </etc/v2cross/nginx_error.log; then
                updateSELinuxHTTPPortT
            fi
        else
...
installCronTLS() {
    if [[ -z "${btDomain}" ]]; then
        ...
        crontab -l >/etc/v2cross/backup_crontab.cron
        local historyCrontab
        historyCrontab=$(sed '/v2cross/d;/acme.sh/d' /etc/v2cross/backup_crontab.cron)
        echo "${historyCrontab}" >/etc/v2cross/backup_crontab.cron
        echo "30 1 * * * /bin/bash /etc/v2cross/install.sh RenewTLS >> /etc/v2cross/crontab_tls.log 2>&1" >>/etc/v2cross/backup_crontab.cron
        crontab /etc/v2cross/backup_crontab.cron
        echoContent green "\n ---> 添加定时维护证书成功"
    fi
}
# 定时任务更新geo文件
installCronUpdateGeo() {
    if [[ -n "${configPath}" ]]; then
        ...
        echoContent skyBlue "\n进度 1/1 : 添加定时更新geo文件"
        crontab -l >/etc/v2cross/backup_crontab.cron
        echo "35 1 * * * /bin/bash /etc/v2cross/install.sh UpdateGeo >> /etc/v2cross/crontab_tls.log 2>&1" >>/etc/v2cross/backup_crontab.cron
        crontab /etc/v2cross/backup_crontab.cron
        echoContent green "\n ---> 添加定时更新geo文件成功"
    fi
}
...
renewalTLS() {
    ...
    if [[ -f "/etc/v2cross/tls/ssl_type" ]]; then
        if grep -q "buypass" <"/etc/v2cross/tls/ssl_type"; then
            sslRenewalDays=180
        fi
    fi
    if [[ -d "$HOME/.acme.sh/${domain}_ecc" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] || [[ "${installDNSACMEStatus}" == "true" ]]; then
        ...
        if [[ ${remainingDays} -le 1 ]]; then
            ...
            sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${domain}" --fullchainpath /etc/v2cross/tls/"${domain}.crt" --keypath /etc/v2cross/tls/"${domain}.key" --ecc
            reloadCore
            handleNginx start
        else
...
installV2Ray() {
    ...
    if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip"
        #        else
        #            wget -c -P /etc/v2cross/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip" >/dev/null 2>&1
        #        fi

        unzip -o "/etc/v2cross/v2ray/${v2rayCoreCPUVendor}.zip" -d /etc/v2cross/v2ray >/dev/null
        rm -rf "/etc/v2cross/v2ray/${v2rayCoreCPUVendor}.zip"
    else
        if [[ "${selectCoreType}" == "3" ]]; then
            echoContent green " ---> 锁定v2ray-core版本为v4.32.1"
            rm -f /etc/v2cross/v2ray/v2ray
            rm -f /etc/v2cross/v2ray/v2ctl
            installV2Ray "$1"
        else
            echoContent green " ---> v2ray-core版本:$(/etc/v2cross/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
    ...
    if [[ -z "${hysteriaConfigPath}" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/hysteria/ "https://github.com/apernet/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}"
        mv "/etc/v2cross/hysteria/${hysteriaCoreCPUVendor}" /etc/v2cross/hysteria/hysteria
        chmod 655 /etc/v2cross/hysteria/hysteria
    else
        echoContent green " ---> Hysteria版本:$(/etc/v2cross/hysteria/hysteria version | grep "Version:" | awk '{print $2}')"
        read -r -p "是否更新、升级？[y/n]:" reInstallHysteriaStatus
        if [[ "${reInstallHysteriaStatus}" == "y" ]]; then
            rm -f /etc/v2cross/hysteria/hysteria
            installHysteria "$1"
        fi
    fi
...
installSingBox() {
    ...
    if [[ -z "${singBoxConfigPath}" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/sing-box/ "https://github.com/SagerNet/sing-box/releases/download/${version}/sing-box-${version/v/}${singBoxCoreCPUVendor}.tar.gz"

        tar zxvf "/etc/v2cross/sing-box/sing-box-${version/v/}${singBoxCoreCPUVendor}.tar.gz" -C "/etc/v2cross/sing-box/" >/dev/null 2>&1

        mv "/etc/v2cross/sing-box/sing-box-${version/v/}${singBoxCoreCPUVendor}/sing-box" /etc/v2cross/sing-box/sing-box
        rm -rf /etc/v2cross/sing-box/sing-box-*
        chmod 655 /etc/v2cross/sing-box/sing-box

    else
        echoContent green " ---> sing-box版本:v$(/etc/v2cross/sing-box/sing-box version | grep "sing-box version" | awk '{print $3}')"
        read -r -p "是否更新、升级？[y/n]:" reInstallSingBoxStatus
        if [[ "${reInstallSingBoxStatus}" == "y" ]]; then
            rm -f /etc/v2cross/sing-box/sing-box
            installSingBox "$1"
        fi
    fi
...
installTuic() {
    ...
    if [[ -z "${tuicConfigPath}" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/tuic/ "https://github.com/EAimTY/tuic/releases/download/${version}/${version}${tuicCoreCPUVendor}"
        mv "/etc/v2cross/tuic/${version}${tuicCoreCPUVendor}" /etc/v2cross/tuic/tuic
        chmod 655 /etc/v2cross/tuic/tuic
    else
        echoContent green " ---> Tuic版本:$(/etc/v2cross/tuic/tuic -v)"
        read -r -p "是否更新、升级？[y/n]:" reInstallTuicStatus
        if [[ "${reInstallTuicStatus}" == "y" ]]; then
            rm -f /etc/v2cross/tuic/tuic
            tuicConfigPath=
            installTuic "$1"
        fi
    fi
...
installXray() {
    ...
    if [[ "${coreInstallType}" != "1" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/xray/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${xrayCoreCPUVendor}.zip"
        if [[ ! -f "/etc/v2cross/xray/${xrayCoreCPUVendor}.zip" ]]; then
            echoContent red " ---> 核心下载失败，请重新尝试安装"
            exit 0
        fi

        unzip -o "/etc/v2cross/xray/${xrayCoreCPUVendor}.zip" -d /etc/v2cross/xray >/dev/null
        rm -rf "/etc/v2cross/xray/${xrayCoreCPUVendor}.zip"

        version=$(curl -s https://api.github.com/repos/Loyalsoldier/v2ray-rules-dat/releases?per_page=1 | jq -r '.[]|.tag_name')
        echoContent skyBlue "------------------------Version-------------------------------"
        echo "version:${version}"
        rm /etc/v2cross/xray/geo* >/dev/null 2>&1

        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/xray/ "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/${version}/geosite.dat"
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/xray/ "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/${version}/geoip.dat"

        chmod 655 /etc/v2cross/xray/xray
    else
        echoContent green " ---> Xray-core版本:$(/etc/v2cross/xray/xray --version | awk '{print $2}' | head -1)"
        read -r -p "是否更新、升级？[y/n]:" reInstallXrayStatus
        if [[ "${reInstallXrayStatus}" == "y" ]]; then
            rm -f /etc/v2cross/xray/xray
            installXray "$1" "$2"
        fi
    fi
}
...
updateV2Ray() {
    ...
    if [[ -z "${coreInstallType}" ]]; then
        ...
        echoContent green " ---> 当前v2ray-core版本:$(/etc/v2cross/v2ray/v2ray --version | awk '{print $2}' | head -1)"
        ...
        if [[ -n "$1" ]]; then
            ...
            if [[ "${rollbackV2RayStatus}" == "y" ]]; then
                if [[ "${coreInstallType}" == "2" ]]; then
                    ...
                    echoContent green " ---> 当前Xray-core版本:$(/etc/v2cross/xray/xray --version | awk '{print $2}' | head -1)"
                fi
        ...
        elif [[ "${version}" == "v$(/etc/v2cross/v2ray/v2ray --version | awk '{print $2}' | head -1)" ]]; then
...
updateXray() {
    ...
    if [[ -z "${coreInstallType}" ]]; then
        ...
        elif [[ "${version}" == "v$(/etc/v2cross/xray/xray --version | awk '{print $2}' | head -1)" ]]; then
...
#        execStart='/etc/v2cross/v2ray/v2ray -confdir /etc/v2cross/v2ray/conf'
...
installHysteriaService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2cross/hysteria/hysteria server -c /etc/v2cross/hysteria/conf/config.json --log-level debug'
...
#        execStart='/etc/v2cross/tuic/tuic -c /etc/v2cross/tuic/conf/config.json'
...
installSingBoxService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2cross/sing-box/sing-box run -c /etc/v2cross/sing-box/conf/config.json'
...
installXrayService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2cross/xray/xray run -confdir /etc/v2cross/xray/conf'
...
#            echoContent red "请手动执行【/etc/v2cross/v2ray/v2ray -confdir /etc/v2cross/v2ray/conf】，查看错误日志"
#            exit 0
#        fi
#    elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
            ...
            echoContent red "请手动执行【/etc/v2cross/hysteria/hysteria --log-level debug -c /etc/v2cross/hysteria/conf/config.json server】，查看错误日志"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleTuic() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "tuic/tuic") ]]; then
            ...
            echoContent red "请手动执行【/etc/v2cross/tuic/tuic -c /etc/v2cross/tuic/conf/config.json】，查看错误日志"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleSingBox() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "sing-box") ]]; then
            ...
            echoContent red "请手动执行【/etc/v2cross/sing-box/sing-box run -c /etc/v2cross/sing-box/conf/config.json】，查看错误日志"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleXray() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "xray/xray") ]]; then
            ...
            echoContent red "请手动执行以下的命令后【/etc/v2cross/xray/xray -confdir /etc/v2cross/xray/conf】将错误日志进行反馈"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
initHysteriaNetwork() {
    ...
    cat <<EOF >/etc/v2cross/hysteria/conf/client_network.json
{
...
hysteriaPortHopping() {
    ...
    if [[ -z "${hysteriaPortHoppingRange}" ]]; then
        ...
        if [[ -z "${portStart}" || -z "${portEnd}" ]]; then
            ...
            #                hysteriaPortHopping
            #            else
            iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "pawdroid_portHopping" -j DNAT --to-destination :${hysteriaPort}

            if iptables-save | grep -q "pawdroid_portHopping"; then
...
readHysteriaPortHopping() {
    if [[ -n "${hysteriaPort}" ]]; then
        ...
        if iptables-save | grep -q "pawdroid_portHopping"; then
            portHopping=
            portHopping=$(iptables-save | grep "pawdroid_portHopping" | cut -d " " -f 8)
...
deleteHysteriaPortHoppingRules() {
    iptables -t nat -L PREROUTING --line-numbers | grep "pawdroid_portHopping" | awk '{print $1}' | while read -r line; do
...
initHysteriaConfig() {
    ...
    cat <<EOF >/etc/v2cross/hysteria/conf/config.json
{
    "listen":":${hysteriaPort}",
    "tls":{
        "cert": "/etc/v2cross/tls/${currentHost}.crt",
        "key": "/etc/v2cross/tls/${currentHost}.key"
    },
    "auth":{
        "type": "password",
        "password": "${uuid}"
...
EOF
    ...
    #    addClientsHysteria "/etc/v2cross/hysteria/conf/config.json" true
...
#    initTuicPort
#    initTuicProtocol
#    cat <<EOF >/etc/v2cross/tuic/conf/config.json
#{
#    "server": "[::]:${tuicPort}",
#    "users": $(initXrayClients 9),
#    "certificate": "/etc/v2cross/tls/${currentHost}.crt",
#    "private_key": "/etc/v2cross/tls/${currentHost}.key",
...
initSingBoxTuicConfig() {
    ...
    initTuicPort
    initTuicProtocol
    cat <<EOF >/etc/v2cross/sing-box/conf/config/tuic.json
{
     "inbounds": [
    {
        "type": "tuic",
        ...
        "tls": {
            ...
            "certificate_path": "/etc/v2cross/tls/${currentHost}.crt",
            "key_path": "/etc/v2cross/tls/${currentHost}.key"
        }
    }
]
}
...
initSingBoxSocks5OutboundsConfig() {
    local uuid=
    uuid=$(/etc/v2cross/xray/xray uuid)
    cat <<EOF >/etc/v2cross/sing-box/conf/config/socks5_outbounds.json
{
     "outbounds": [
    {
        "type": "socks",
...
initSingBoxHysteria2Config() {
    ...
    initHysteriaPort
    # todo 123123
    cat <<EOF >/etc/v2cross/sing-box/conf/config/hysteria2.json
{
    "inbounds": [
        {
            "type": "hysteria2",
...
# 合并config
singBoxMergeConfig() {
    rm /etc/v2cross/sing-box/conf/config.json >/dev/null 2>&1
    /etc/v2cross/sing-box/sing-box merge config.json -C /etc/v2cross/sing-box/conf/config/ -D /etc/v2cross/sing-box/conf/ >/dev/null 2>&1
}
...
initV2RayConfig() {
    ...
    if [[ -n "${currentUUID}" && -z "${uuid}" ]]; then
        ...
        if [[ "${historyUUIDStatus}" == "y" ]]; then
            ...
            uuid=$(/etc/v2cross/v2ray/v2ctl uuid)
        fi
    elif [[ -z "${uuid}" ]]; then
        uuid=$(/etc/v2cross/v2ray/v2ctl uuid)
    fi
    ...
    movePreviousConfig
    # log
    cat <<EOF >/etc/v2cross/v2ray/conf/00_log.json
{
  "log": {
    "error": "/etc/v2cross/v2ray/error.log",
    "loglevel": "warning"
  }
}
EOF
    # outbounds
    if [[ -n "${pingIPv6}" ]]; then
        cat <<EOF >/etc/v2cross/v2ray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
        ...
        cat <<EOF >/etc/v2cross/v2ray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
    ...
    # dns
    cat <<EOF >/etc/v2cross/v2ray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
    ...
    if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2cross/v2ray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
        addClients "/etc/v2cross/v2ray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2cross/v2ray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
        addClients "/etc/v2cross/v2ray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2cross/v2ray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
        addClients "/etc/v2cross/v2ray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2cross/v2ray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
        addClients "/etc/v2cross/v2ray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
    fi

    if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
        getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
        cat <<EOF >/etc/v2cross/v2ray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
        addClients "/etc/v2cross/v2ray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
    fi
    ...
    cat <<EOF >/etc/v2cross/v2ray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
  ...
  "streamSettings": {
    ...
    "tlsSettings": {
      ...
      "certificates": [
        {
          "certificateFile": "/etc/v2cross/tls/${domain}.crt",
          "keyFile": "/etc/v2cross/tls/${domain}.key",
          "ocspStapling": 3600,
          "usage":"encipherment"
        }
      ]
...
EOF
    addClients "/etc/v2cross/v2ray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
...
initXrayConfig() {
    ...
    # log
    if [[ ! -f "/etc/v2cross/xray/conf/00_log.json" ]]; then

        cat <<EOF >/etc/v2cross/xray/conf/00_log.json
{
  "log": {
    "error": "/etc/v2cross/xray/error.log",
    "loglevel": "warning"
  }
}
EOF
    ...
    if [[ ! -f "/etc/v2cross/xray/conf/12_policy.json" ]]; then

        cat <<EOF >/etc/v2cross/xray/conf/12_policy.json
{
  "policy": {
      "levels": {
          "0": {
...
EOF
    ...
    # outbounds
    if [[ ! -f "/etc/v2cross/xray/conf/10_ipv6_outbounds.json" ]]; then
        if [[ -n "${pingIPv6}" ]]; then
            cat <<EOF >/etc/v2cross/xray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
            ...
            cat <<EOF >/etc/v2cross/xray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
    ...
    # dns
    if [[ ! -f "/etc/v2cross/xray/conf/11_dns.json" ]]; then
        cat <<EOF >/etc/v2cross/xray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
    ...
    # routing
    if [[ ! -f "/etc/v2cross/xray/conf/09_routing.json" ]]; then
        cat <<EOF >/etc/v2cross/xray/conf/09_routing.json
{
  "routing": {
    "rules": [
      {
...
EOF
    ...
    if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
        fallbacksList='{"dest":31296,"xver":1},{"alpn":"h2","dest":31302,"xver":0}'
        cat <<EOF >/etc/v2cross/xray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
        ...
        rm /etc/v2cross/xray/conf/04_trojan_TCP_inbounds.json >/dev/null 2>&1
    fi

    # VLESS_WS_TLS
    if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
        fallbacksList=${fallbacksList}',{"path":"/'${customPath}'ws","dest":31297,"xver":1}'
        cat <<EOF >/etc/v2cross/xray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
        ...
        rm /etc/v2cross/xray/conf/03_VLESS_WS_inbounds.json >/dev/null 2>&1
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2cross/xray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
        ...
        rm /etc/v2cross/xray/conf/04_trojan_gRPC_inbounds.json >/dev/null 2>&1
    fi

    # VMess_WS
    if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
        fallbacksList=${fallbacksList}',{"path":"/'${customPath}'vws","dest":31299,"xver":1}'
        cat <<EOF >/etc/v2cross/xray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
        ...
        rm /etc/v2cross/xray/conf/05_VMess_WS_inbounds.json >/dev/null 2>&1
    fi

    if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
        cat <<EOF >/etc/v2cross/xray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
        ...
        rm /etc/v2cross/xray/conf/06_VLESS_gRPC_inbounds.json >/dev/null 2>&1
    fi
    # VLESS Vision
    if echo "${selectCustomInstallType}" | grep -q 0 || [[ "$1" == "all" ]]; then

        cat <<EOF >/etc/v2cross/xray/conf/02_VLESS_TCP_inbounds.json
{
    "inbounds":[
        {
          "port": ${port},
...
EOF
        ...
        rm /etc/v2cross/xray/conf/02_VLESS_TCP_inbounds.json >/dev/null 2>&1
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 7 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2cross/xray/conf/07_VLESS_vision_reality_inbounds.json
{
  "inbounds": [
    {
      "port": ${realityPort},
...
EOF
        ...
        cat <<EOF >/etc/v2cross/xray/conf/08_VLESS_reality_fallback_grpc_inbounds.json
{
  "inbounds": [
    {
      "port": 31305,
...
EOF
        ...
        rm /etc/v2cross/xray/conf/07_VLESS_vision_reality_inbounds.json >/dev/null 2>&1
        rm /etc/v2cross/xray/conf/08_VLESS_reality_fallback_grpc_inbounds.json >/dev/null 2>&1
    fi
    installSniffing
}
# 初始化Xray Reality配置
...
customCDNIP() {
    ...
    echoContent yellow "# 注意事项"
    echoContent yellow "\n教程地址:"
    echoContent skyBlue "https://v2cross.com/archives/5008"
...
defaultBase64Code() {
    ...
    if [[ "${type}" == "vlesstcp" ]]; then
        ...
        if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
            ...
            cat <<EOF >>"/etc/v2cross/subscribe_local/default/${user}"
...
EOF
            cat <<EOF >>"/etc/v2cross/subscribe_local/clashMeta/${user}"
  - name: "${email}"
    type: vless
    server: ${currentHost}
    port: ${currentDefaultPort}
...
EOF
        ...
        echo "{\"relay\": {\"server\": \"${currentHost}:${tuicPort}\",\"uuid\": \"${id}\",\"password\": \"${id}\",\"ip\": \"$(getPublicIP 4)\",\"congestion_control\": \"${tuicAlgorithm}\",\"alpn\": [\"h3\"]},\"local\": {\"server\": \"127.0.0.1:7798\"},\"log_level\": \"warn\"}" | jq
...
backupNginxConfig() {
    if [[ "$1" == "backup" ]]; then
        cp ${nginxConfigPath}alone.conf /etc/v2cross/alone_backup.conf
        echoContent green " ---> nginx配置文件备份成功"
    fi

    if [[ "$1" == "restoreBackup" ]] && [[ -f "/etc/v2cross/alone_backup.conf" ]]; then
        cp /etc/v2cross/alone_backup.conf ${nginxConfigPath}alone.conf
        echoContent green " ---> nginx配置文件恢复备份成功"
        rm /etc/v2cross/alone_backup.conf
    fi
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/Pawdroid/shell/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
...
unInstall() {
    ...
    #    rm -rf /tmp/v2cross-tls/*
    #    if [[ -d "/etc/v2cross/tls" ]] && [[ -n $(find /etc/v2cross/tls/ -name "*.key") ]] && [[ -n $(find /etc/v2cross/tls/ -name "*.crt") ]]; then
    #        mv /etc/v2cross/tls /tmp/v2cross-tls
    #        if [[ -n $(find /tmp/v2cross-tls -name '*.key') ]]; then
    #            echoContent yellow " ---> 备份证书成功，请注意留存。[/tmp/v2cross-tls]"
    #        fi
    #    fi
    ...
    rm -rf /usr/bin/cross
    rm -rf /usr/sbin/cross
    echoContent green " ---> 卸载快捷方式完成"
    echoContent green " ---> 卸载v2cross脚本完成"
}
...
customUUID() {
    ...
    if [[ -z "${currentCustomUUID}" ]]; then
        ...
        if [[ -f "/tmp/v2cross" && -n $(cat /tmp/v2cross) ]]; then
...
# 更新脚本
updateScript() {
    echoContent skyBlue "\n进度  $1/${totalProgress} : 更新v2cross脚本"
    rm -rf /etc/v2cross/install.sh
    #    if wget --help | grep -q show-progress; then
    wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/ -N --no-check-certificate "https://raw.githubusercontent.com/Pawdroid/shell/master/install.sh"
    #    else
    #        wget -c -q -P /etc/v2cross/ -N --no-check-certificate "https://raw.githubusercontent.com/Pawdroid/shell/master/install.sh"
    #    fi

    sudo chmod 700 /etc/v2cross/install.sh
    local version
    version=$(grep '当前版本：v' "/etc/v2cross/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F "[\"]" '{print $1}')

    echoContent green "\n ---> 更新完毕"
    echoContent yellow " ---> 请手动执行[cross]打开脚本"
    echoContent green " ---> 当前版本：${version}\n"
    echoContent yellow "如更新不成功，请手动执行下面命令\n"
    echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/Pawdroid/shell/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
...
EOF
    ...
    3)
        ...
        ;;
    4)
        if [[ ! -f "/etc/v2cross/crontab_tls.log" ]]; then
            touch /etc/v2cross/crontab_tls.log
        fi
        tail -n 100 /etc/v2cross/crontab_tls.log
        ;;
    5)
        tail -n 100 /etc/v2cross/tls/acme.log
        ;;
...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2cross" ]] && grep <"$HOME/install.sh" -q ""; then
        mv "$HOME/install.sh" /etc/v2cross/install.sh
        local crossType=
        if [[ -d "/usr/bin/" ]]; then
            if [[ ! -f "/usr/bin/cross" ]]; then
                ln -s /etc/v2cross/install.sh /usr/bin/cross
                chmod 700 /usr/bin/cross
                crossType=true
            fi

            rm -rf "$HOME/install.sh"
        elif [[ -d "/usr/sbin" ]]; then
            if [[ ! -f "/usr/sbin/cross" ]]; then
                ln -s /etc/v2cross/install.sh /usr/sbin/cross
                chmod 700 /usr/sbin/cross
                crossType=true
            fi
            rm -rf "$HOME/install.sh"
        fi
        if [[ "${crossType}" == "true" ]]; then
            echoContent green "快捷方式创建成功，可执行[cross]重新打开脚本"
        fi
    fi
}
...
ipv6Routing() {
    ...
    if [[ "${ipv6Status}" == "1" ]]; then
        ...
        echoContent yellow "# 注意事项\n"
        echoContent yellow "# 注意事项"
        echoContent yellow "# 使用教程：https://v2cross.com/archives/5014 \n"
...
# 读取第三方warp配置
readConfigWarpReg() {
    if [[ ! -f "/etc/v2cross/warp/config" ]]; then
        /etc/v2cross/warp/warp-reg >/etc/v2cross/warp/config
    fi

    secretKeyWarpReg=$(grep <"/etc/v2cross/warp/config" private_key | awk '{print $2}')

    addressWarpReg=$(grep <"/etc/v2cross/warp/config" v6 | awk '{print $2}')

    publicKeyWarpReg=$(grep <"/etc/v2cross/warp/config" public_key | awk '{print $2}')

    reservedWarpReg=$(grep <"/etc/v2cross/warp/config" reserved | awk -F "[:]" '{print $2}')
...
warpRoutingReg() {
    ...
    echoContent red "=============================================================="
    if [[ ! -f "/etc/v2cross/warp/warp-reg" ]]; then
        echo
        echoContent yellow "# 注意事项"
        ...
        if [[ "${installWarpRegStatus}" == "y" ]]; then
            ...
            curl -sLo /etc/v2cross/warp/warp-reg "https://github.com/badafans/warp-reg/releases/download/v1.0/${warpRegCoreCPUVendor}"
            chmod 655 /etc/v2cross/warp/warp-reg
...
EOF
        ...
        if [[ "${type}" == "IPv4" ]]; then
            if ! grep -q "wireguard-out-IPv6" <${configPath}10_ipv4_outbounds.json; then
                rm -rf /etc/v2cross/warp/config >/dev/null 2>&1
            fi
        elif [[ "${type}" == "IPv6" ]]; then
...
unInstallXrayCoreReality() {
    ...
    handleXray stop
    rm /etc/v2cross/xray/conf/07_VLESS_vision_reality_inbounds.json
    rm /etc/v2cross/xray/conf/08_VLESS_reality_fallback_grpc_inbounds.json
    echoContent green " ---> 卸载完成"
}
...
cronFunction() {
    if [[ "${cronName}" == "RenewTLS" ]]; then
        ...
        updateGeoSite >>/etc/v2cross/crontab_updateGeoSite.log
        echoContent green " ---> geo更新日期:$(date "+%F %H:%M:%S")" >>/etc/v2cross/crontab_updateGeoSite.log
        exit 0
    fi
}
# 账号管理
...
addSubscribeMenu() {
    ...
    if [[ "${addSubscribeStatus}" == "1" ]]; then
        ...
        rm -rf /etc/v2cross/subscribe_remote/clashMeta/*
        rm -rf /etc/v2cross/subscribe_remote/default/*
        echo >/etc/v2cross/subscribe_remote/remoteSubscribeUrl
...
addOtherSubscribe() {
    echoContent yellow "#注意事项:"
    echoContent yellow "请仔细阅读以下文章： https://v2cross.com/archives/5005"
    echoContent skyBlue "录入示例：www.v2cross.com:443:vps1\n"
    ...
    if [[ -z "${remoteSubscribeUrl}" ]]; then
        ...
        echo "${remoteSubscribeUrl}" >>/etc/v2cross/subscribe_remote/remoteSubscribeUrl
        local remoteUrl=
        ...
        if [[ -n $(ls /etc/v2cross/subscribe/clashMeta/) || -n $(ls /etc/v2cross/subscribe/default/) ]]; then
            find /etc/v2cross/subscribe_local/default/* | while read -r email; do
                email=$(echo "${email}" | awk -F "[d][e][f][a][u][l][t][/]" '{print $2}')

                local emailMd5=
                emailMd5=$(echo -n "${email}$(cat "/etc/v2cross/subscribe_local/subscribeSalt")"$'\n' | md5sum | awk '{print $1}')
                ...
                if echo "${default}" | grep -q "${email}"; then
                    echo "${default}" >>"/etc/v2cross/subscribe/default/${emailMd5}"
                    echo "${default}" >>"/etc/v2cross/subscribe_remote/default/${email}"
                ...
                if echo "${clashMetaProxies}" | grep -q "${email}"; then
                    echo "${clashMetaProxies}" >>"/etc/v2cross/subscribe/clashMeta/${emailMd5}"
                    echo "${clashMetaProxies}" >>"/etc/v2cross/subscribe_remote/clashMeta/${email}"
...
clashMetaConfig() {
    local url=$1
    local id=$2
    cat <<EOF >"/etc/v2cross/subscribe/clashMetaProfiles/${id}"
mixed-port: 7890
unified-delay: false
geodata-mode: true
...
subscribe() {
    ...
    if echo "${currentInstallProtocolType}" | grep -q 0 && [[ -n "${configPath}" ]]; then
        ...
        if [[ -f "/etc/v2cross/subscribe_local/subscribeSalt" && -n $(cat "/etc/v2cross/subscribe_local/subscribeSalt") ]]; then
            read -r -p "读取到上次安装设置的Salt，是否使用上次生成的Salt ？[y/n]:" historySaltStatus
            if [[ "${historySaltStatus}" == "y" ]]; then
                subscribeSalt=$(cat /etc/v2cross/subscribe_local/subscribeSalt)
            else
        ...
        echo "${subscribeSalt}" >/etc/v2cross/subscribe_local/subscribeSalt

        rm -rf /etc/v2cross/subscribe/default/*
        rm -rf /etc/v2cross/subscribe/clashMeta/*
        rm -rf /etc/v2cross/subscribe_local/default/*
        rm -rf /etc/v2cross/subscribe_local/clashMeta/*
        showAccounts >/dev/null

        if [[ -n $(ls /etc/v2cross/subscribe_local/default/) ]]; then
            find /etc/v2cross/subscribe_local/default/* | while read -r email; do
                email=$(echo "${email}" | awk -F "[d][e][f][a][u][l][t][/]" '{print $2}')
                # md5加密
                local emailMd5=
                ...
                cat "/etc/v2cross/subscribe_local/default/${email}" >>"/etc/v2cross/subscribe/default/${emailMd5}"

                if [[ -f "/etc/v2cross/subscribe_remote/default/${email}" ]]; then
                    echo >"/etc/v2cross/subscribe_remote/default/${email}_tmp"
                    while read -r remoteUrl; do
                        updateRemoteSubscribe "${emailMd5}" "${email}" "${remoteUrl}" "default"
                    done < <(grep "VLESS_TCP/TLS_Vision" <"/etc/v2cross/subscribe_remote/default/${email}" | awk -F "@" '{print $2}' | awk -F "?" '{print $1}')

                    echo >"/etc/v2cross/subscribe_remote/default/${email}"
                    cat "/etc/v2cross/subscribe_remote/default/${email}_tmp" >"/etc/v2cross/subscribe_remote/default/${email}"
                    cat "/etc/v2cross/subscribe_remote/default/${email}" >>"/etc/v2cross/subscribe/default/${emailMd5}"
                fi

                local base64Result
                base64Result=$(base64 -w 0 "/etc/v2cross/subscribe/default/${emailMd5}")
                echo "${base64Result}" >"/etc/v2cross/subscribe/default/${emailMd5}"
                ...
                # clashMeta
                if [[ -f "/etc/v2cross/subscribe_local/clashMeta/${email}" ]]; then

                    cat "/etc/v2cross/subscribe_local/clashMeta/${email}" >>"/etc/v2cross/subscribe/clashMeta/${emailMd5}"

                    if [[ -f "/etc/v2cross/subscribe_remote/clashMeta/${email}" ]]; then
                        echo >"/etc/v2cross/subscribe_remote/clashMeta/${email}_tmp"
                        while read -r remoteUrl; do
                            updateRemoteSubscribe "${emailMd5}" "${email}" "${remoteUrl}" "ClashMeta"
                        done < <(grep -A3 "VLESS_TCP/TLS_Vision" <"/etc/v2cross/subscribe_remote/clashMeta/${email}" | awk '/server:|port:/ {print $2}' | paste -d ':' - -)
                        echo >"/etc/v2cross/subscribe_remote/clashMeta/${email}"
                        cat "/etc/v2cross/subscribe_remote/clashMeta/${email}_tmp" >"/etc/v2cross/subscribe_remote/clashMeta/${email}"
                        cat "/etc/v2cross/subscribe_remote/clashMeta/${email}" >>"/etc/v2cross/subscribe/clashMeta/${emailMd5}"
                    fi

                    sed -i '1i\proxies:' "/etc/v2cross/subscribe/clashMeta/${emailMd5}"
...
updateRemoteSubscribe() {
    ...
    local serverAlias=
    serverAlias=$(grep "${remoteDomain}" <"/etc/v2cross/subscribe_remote/remoteSubscribeUrl" | awk -F ":" '{print $3}')

    if [[ "${type}" == "ClashMeta" ]]; then
        local clashMetaProxies=
        clashMetaProxies=$(curl -s -4 "https://${remoteUrl}/s/clashMeta/${emailMD5}" | sed '/proxies:/d' | sed "s/${email}/${email}_${serverAlias}/g")
        if echo "${clashMetaProxies}" | grep -q "${email}"; then
            echo "${clashMetaProxies}" >>"/etc/v2cross/subscribe_remote/clashMeta/${email}_tmp"
        ...
        default=$(curl -s -4 "https://${remoteUrl}/s/default/${emailMD5}" | base64 -d | sed "s/${email}/${email}_${serverAlias}/g")
        if echo "${default}" | grep -q "${email}"; then
            echo "${default}" >>"/etc/v2cross/subscribe_remote/default/${email}_tmp"
...
initRealityKey() {
    ...
    if [[ -z "${realityPrivateKey}" ]]; then
        realityX25519Key=$(/etc/v2cross/xray/xray x25519)
...
# sing-box log日志
singBoxLog() {
    cat <<EOF >/etc/v2cross/sing-box/conf/config/log.json
        {
          "log": {
            "disabled": $1,
            "level": "debug",
            "output": "/etc/v2cross/sing-box/box.log",
            "timestamp": true
          }
        }
EOF
...
hysteriaVersionManageMenu() {
    ...
    if [[ ! -d "/etc/v2cross/hysteria/" ]]; then
        echoContent red " ---> 没有检测到安装目录，请执行脚本安装内容"
        menu
        exit 0
    fi
...
singBoxVersionManageMenu() {
    ...
    if [[ ! -f "/etc/v2cross/sing-box/sing-box" ]]; then
...
menu() {
    cd "$HOME" || exit
    echoContent red "\n=============================================================="
    echoContent green "当前版本：v8.1.20"
    echoContent green "Github：https://github.com/Pawdroid/shell"
    echoContent green "描述：八合一共存脚本\c"
    showInstallStatus
    checkWgetShowProgress
    echoContent red "\n=========================== 更多 ============================"
    echoContent red "                                              "
    echoContent green "TG群：@shadowrocket_android\n"
    echoContent green "官网：https://v2cross.com"
    echoContent red "=============================================================="
    if [[ -n "${coreInstallType}" ]]; then
        echoContent yellow "1.重新安装"
    else
