...
# 恢复配置
resetNginxConfig(){
    `cp -Rrf /tmp/pawdroid/nginx/nginx.conf /etc/nginx/nginx.conf`
    rm -rf /etc/nginx/conf.d/5NX2O9XQKP.conf
    echoColor green "\n恢复配置完毕"
}
# 备份
bakConfig(){
    mkdir -p /tmp/pawdroid/nginx
    `cp -Rrf /etc/nginx/nginx.conf /tmp/pawdroid/nginx/nginx.conf`
}
# 安装证书
installTLS(){
    echoColor yellow "请输入域名【例:xxx.v2cross.com】："
    read domain
    if [[ -z ${domain} ]]
    then
        echoColor red "域名未填写\n"
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} --fullchainpath /tmp/pawdroid/nginx/${domain}.crt --keypath /tmp/pawdroid/nginx/${domain}.key --ecc >/dev/null
    if [[ -z `cat /tmp/pawdroid/nginx/${domain}.key` ]]
    then
        echoColor red "证书key生成失败，请重新运行"
        resetNginxConfig
        exit
    elif [[ -z `cat /tmp/pawdroid/nginx/${domain}.crt` ]]
    then
    ...
    echoColor green "证书目录/tmp/pawdroid/nginx"
    ls /tmp/pawdroid/nginx
...
init(){
    ...
    echoColor green "   12.Github[https://github.com/Pawdroid]"
    echoColor red "=============================="
