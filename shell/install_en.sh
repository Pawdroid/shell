...
readInstallType() {
    ...
    if [[ -d "/etc/v2cross" ]]; then
        ...
        if [[ -d "/etc/v2cross/hysteria" && -f "/etc/v2cross/hysteria/hysteria" ]]; then
            ...
            if [[ -d "/etc/v2cross/hysteria/conf" ]] && [[ -f "/etc/v2cross/hysteria/conf/config.json" ]] && [[ -f "/etc/v2cross/hysteria/conf/client_network.json" ]]; then
...
initTLSNginxConfig() {
    ...
    if [[ -n "${currentHost}" ]]; then
        ...
        if [[ "${historyDomainStatus}" == "y" ]]; then
            ...
            echo
            echoContent yellow "Please enter the domain name to be configured: www.v2cross.com --->"
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        if [[ -n "${txtValue}" ]]; then
            ...
            echoContent yellow " ---> Please refer to this tutorial for adding method, https://github.com/mack-a/v2cross/blob/master/documents/dns_txt.md"
...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            wget -q -P "${nginxStaticPath}" https://raw.githubusercontent.com/mack-a/v2cross/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
installV2Ray() {
    ...
    if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
        ...
        if [[ "${selectCoreType}" == "3" ]]; then
            ...
            echoContent green " ---> v2ray-core version:$(/etc/v2cross/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
    ...
    if [[ -z "${hysteriaConfigPath}" ]]; then
        ...
        echoContent green " ---> Hysteria version:$(/etc/v2cross/hysteria/hysteria --version | awk '{print $3}')"
...
installTuic() {
    ...
    if [[ -z "${tuicConfigPath}" ]]; then
        ...
        echoContent green " ---> Tuic version:$(/etc/v2cross/tuic/tuic -v)"
...
installXray() {
    ...
    if [[ "${coreInstallType}" != "1" ]]; then
        ...
        echoContent green " ---> Xray-core version:$(/etc/v2cross/xray/xray --version | awk '{print $2}' | head -1)"
...
updateV2Ray() {
    ...
    if [[ -z "${coreInstallType}" ]]; then
        ...
        echoContent green " ---> Current v2ray-core version: $(/etc/v2cross/v2ray/v2ray --version | awk '{print $2}' | head -1)"
        ...
        if [[ -n "$1" ]]; then
            ...
            if [[ "${rollbackV2RayStatus}" == "y" ]]; then
                if [[ "${coreInstallType}" == "2" ]]; then
                    ...
                    echoContent green " ---> Current Xray-core version: $(/etc/v2cross/xray/xray --version | awk '{print $2}' | head -1)"
                fi
...
installHysteriaService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2cross/hysteria/hysteria --log-level info -c /etc/v2cross/hysteria/conf/config.json server'
...
handleV2Ray() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
            ...
            echoContent red "Please manually execute [/etc/v2cross/v2ray/v2ray -confdir /etc/v2cross/v2ray/conf] and check the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
            ...
            echoContent red "Please manually execute [/etc/v2cross/hysteria/hysteria --log-level debug -c /etc/v2cross/hysteria/conf/config.json server] to view the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleTuic() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "tuic/tuic") ]]; then
            ...
            echoContent red "Please manually execute [/etc/v2cross/tuic/tuic -c /etc/v2cross/tuic/conf/config.json] and check the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleXray() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "xray/xray") ]]; then
            ...
            echoContent red "Please manually execute the following command [/etc/v2cross/xray/xray -confdir /etc/v2cross/xray/conf] and feedback the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
EOF
        ...
        cat <<EOF >"/etc/v2cross/hysteria/conf/client.json"
{
...
EOF
        local v2rayNConf=
        v2rayNConf="$(cat /etc/v2cross/hysteria/conf/client.json)"
...
EOF
        ...
        cat <<EOF >"/etc/v2cross/tuic/conf/v2rayN.json"
{
    "relay": {
...
EOF
        local v2rayNConf=
        v2rayNConf="$(cat /etc/v2cross/tuic/conf/v2rayN.json)"
        echoContent green "${v2rayNConf}"

        cat <<EOF >>"/etc/v2cross/subscribe_local/clashMeta/${email}"
  - name: "${email}_tuic"
    server: ${currentHost}
    type: tuic
    port: ${tuicPort}
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/mack-a/v2cross/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
...
unInstall() {
    ...
    # echoContent yellow " ---> Backup certificate successful, please save it. [/tmp/v2cross-tls]"
    #fi
    #fi
    ...
    echoContent green " ---> Uninstall v2cross script completed"
}
...
# update script
updateV2RayAgent() {
    echoContent skyBlue "\nProgress$1/${totalProgress}: Update v2cross script"
    rm -rf /etc/v2cross/install.sh
    # if wget --help | grep -q show-progress; then
    wget -c -q "${wgetShowProgressStatus}" -P /etc/v2cross/ -N --no-check-certificate "https://raw.githubusercontent.com/mack-a/v2cross/master/install.sh"
    #else
    # wget -c -q -P /etc/v2cross/ -N --no-check-certificate "https://raw.githubusercontent.com/mack-a/v2cross/master/install.sh"
    #fi

    sudo chmod 700 /etc/v2cross/install.sh
    local version
    version=$(grep 'Current version: v' "/etc/v2cross/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F "[\"]" '{print $1}')

    echoContent green "\n ---> Update completed"
    echoContent yellow " ---> Please manually execute [cross] to open the script"
    ...
    echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/mack-a/v2cross/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2cross" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
        ...
        if [[ "${crossType}" == "true" ]]; then
            echoContent green "The shortcut is created successfully, you can execute [cross] to reopen the script"
        fi
    fi
}
...
cronFunction() {
    if [[ "${cronName}" == "RenewTLS" ]]; then
        ...
        echoContent green " ---> geo update date: $(date "+%F %H:%M:%S")" >>/etc/v2cross/crontab_updateGeoSite.log
        exit 0
    fi
}
#Account management
...
addOtherSubscribe() {
    echoContent yellow "#Notes:"
    echoContent skyBlue "Input example: www.v2cross.com:443:vps1\n"
...
menu() {
    ...
    echoContent green "Github: https://github.com/mack-a/v2cross"
    echoContent green "Description: 8-in-1 coexistence script\c"
    showInstallStatus
    checkWgetShowProgress
    echoContent red "\n============================ Promotion area================ ============"
    echoContent red " "
    echoContent green "For promotion, please contact TG: @v2cross\n"
    echoContent red "================================================== ==============="
    if [[ -n "${coreInstallType}" ]]; then
        echoContent yellow "1.Reinstall"
    else
